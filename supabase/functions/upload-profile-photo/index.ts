...
// Without this, the gateway rejects requests missing Authorization.
export const config = {
  verify_jwt: false,
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // IMPORTANT:
    // Supabase Functions gateway may reject non-HS256 JWTs in the Authorization header.
    // Our project issues ES256 (JWKS) JWTs, so we accept the user's JWT via a custom header.
    ...
    // At this point the user's JWT is verified via JWKS and userId is extracted from sub.
    ...
    const publicUrl = admin.storage.from("profile-photos").getPublicUrl(filePath).data.publicUrl;
